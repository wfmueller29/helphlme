...
test_that("hlme2 works within for loop", {
  set.seed(100)
  our_models <- list()
  for (i in seq_len(2)) {
    i <- i
    cat(i, "\n")
    our_models[[i]] <- helphlme::hlme2(
    data = data_hlme,
    fixed = Y ~ X1 + X2 + X3,
    mixture = ~ X1 + X2 + X3,
    random = ~1,
  ...
  check <- all.equal(our_models, their_models)
