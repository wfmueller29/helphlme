...
ran_can <- function(df, fixed, mixture, random_vect, subject, par = F){
  df <- substitute(...(df = df))$df

  if(par ==F){
    ran_can_base(df, fixed, mixture, random_vect, subject)
  } else{
    ran_can_par(df, fixed, mixture, random_vect, subject)
  }
}

ran_can_base <- function(df, fixed, mixture, random_vect, subject){
  mos <- lapply(1: length(random_vect), function(i){
    mo <- lcmem(df = df, fixed = fixed, mixture = mixture, random = random_vect[[i]], subject = subject, k = 5)
  })
  return(mos)
}


ran_can_par <- function(df, fixed, mixture, random_vect, subject){
  mos <- listenv()
  for(i in length(forms)){
    mos[[i]] %<% lcmem(df = df, fixed = fixed, mixture = mixture, random = random_vect[[i]], subject = subject, k = 5)
  }
  mos <- as.list(mos)
  return(mos)
}
